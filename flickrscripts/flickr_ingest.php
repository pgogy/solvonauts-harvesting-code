<?PHP
	...
	class flickr_ingest{
		...
		function flickr_ingest(){	
			...
			include dirname(__FILE__) . "/../../public_html/site/database/database_layer.inc";

			include dirname(__FILE__) . "/../../public_html/site/database/" . DB_TYPE . "_database_layer.inc";
		...
		public function flickr_search($name){			
			...
			if(isset($rsp_obj['user'])){
				...
				if(isset($rsp_obj['photos'])){
					...
					$this->database->insert_query("update oer_site_list set items_harvested = :items_harvested where site_address = :site_address",
